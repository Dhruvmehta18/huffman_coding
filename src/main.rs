        ...
        Ok(file_str) => {
            ...
            if let Some(node) = get_huffman_tree_node(&mut priority_queue) {
                ...
                if let Some(path) = path_buf.parent() {
                    let compress_file_path = path.join(path_buf.file_stem().unwrap().to_str().unwrap().to_owned() + ".huf");
...
fn decode(path: &String) {
    ...
    match &path.parent() {
        ...
        Some(parent_path) => {
            let file_write_path = parent_path.join(path.file_stem().unwrap().to_str().unwrap().to_owned() + "_decode" + ".txt");
...
fn traverse_and_get_prefixes_int(node: & Option<TreeNodeRef>, bits: &mut Vec<bool>, map: &mut HashMap<char, Vec<bool>>) {
